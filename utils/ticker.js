  ...
  */
  ...
  pause (tickObject) {
    if (typeof tickObject === 'undefined') {
      ...
      tickObject.tick._paused = true
      tickObject.tick._lastTime = this._lastTime
    }
  }
  ...
  _tick () {
    ...
    for (let i = 0, len = tickersCopy.length; i < len; i++) {
      // 单个tick的deltaTime
      let deltaTime = startTime - (tickersCopy[i].tick._lastTime || this._lastTime)
      tickersCopy[i].tick._paused || (tickersCopy[i].tick._lastTime = 0) || tickersCopy[i].tick(deltaTime / 1000 + 0.5 >>> 0)
    }
